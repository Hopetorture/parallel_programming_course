...
void md5_t::avx2_md5(const QList<QByteArray> &input){
    ...
    int i=0;
    for(; i<16; i++) {
        //__m256i f = _mm256_xor_si128(d, _mm_and_si128(b, _mm_xor_si128(c, d)));
...
//void md5_t::process(__m256i &input, QVector<const uint> input_length)
//{
//    if (!finished) {
