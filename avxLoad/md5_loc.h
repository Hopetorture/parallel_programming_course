...
namespace md5 {
    ...
    */
   inline unsigned int cyclic_left_rotate(__m128i a, __m128i f,  __m128i kv,  __m128i blockv, __m128i rv) {
       //return (data << shift_bits) | (data >> (32 - shift_bits));
       __m256i x = _mm256_add_epi32(a, _mm256_add_epi32(f, _mm256_add_epi32(kv, blockv)));
       return _mm256_or_si256( _mm256_sll_epi32(x, rv),
                             _mm256_srl_epi32(x, _mm256_sub_epi32( _mm256_set1_epi32(32), rv)));
   }
